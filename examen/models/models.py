...
class Furgoneta(models.Model):
    ...
    volum_disponible = fields.Integer(compute='_get_disponible', store=True)

    viatges = fields.One2many(string='Els meus viatges', comodel_name='examen.viatge', inverse_name='furgoneta', readonly=True)

    # Fields relationals
    paquets = fields.Many2many('examen.paquet', related='viatges.paquets')

    # Funcions compute
    @api.depends('paquets', 'viatges.paquets.volum')
    def _get_disponible(self):
        for f in self:
            volum_total = 0
            for p in f.paquets:
                volum_total = volum_total + p.volum
            if volum_total > f.capacitat:
